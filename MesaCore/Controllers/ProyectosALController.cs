...
namespace MesaCore.Controllers
{
    ...
    [ApiController]
    public class ProyectosALController : ControllerBase
    {
        private readonly AppDbContext _context;

        public ProyectosALController(AppDbContext context)
        {
            _context = context;
        }
        ...
        {
            var projectId = await _context.Proyectosfxsal.FirstOrDefaultAsync(p  => p.Id == id);
        ...
        [HttpPost]
        [Route("Registrar")]
        public async Task<IActionResult> Create([FromBody] Proyectosfxsal proyectos)
        {
            ...
            await _context.Proyectosfxsal.AddAsync(proyectos);
        ...
        [HttpPut]
        [Route("Editar/{id:int}")]
        public async Task<IActionResult> Edit([FromBody] ProyectoUpdateDto dto, int id)
        {
            var proyecto = await _context.Proyectosfxsal.FindAsync(id);
        ...
        {
            var registroId = await _context.Proyectosfxsal.FirstOrDefaultAsync(x => x.Id == id);

            if (registroId == null) return BadRequest("No se encontró el id seleccionado");

            _context.Proyectosfxsal.Remove(registroId);
            await _context.SaveChangesAsync();
